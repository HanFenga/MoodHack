package dev.hanfeng.zhebushigudu.xufangggg.util.gl;
...
public class PNGDecoder
{
    ...
    private void unfilterPaeth(final byte[] curLine, final byte[] prevLine) {
        ...
        int c;
        int n2;
        for (final int n = curLine.length; i < n; n2 = i++, curLine[n2] += (byte)c) {
    ...
    private void readChunkUnzip(Inflater inflater, byte[] buffer, int offset, int length) throws IOException {
        ...
        try {
            ...
            throw (IOException)new IOException("inflate error").initCause((Throwable)ex);
        }
    }
